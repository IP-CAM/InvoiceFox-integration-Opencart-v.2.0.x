...
class ModelInvoiceFoxHooks extends Model { 
  ...
  public function __construct($registry)
  {
    ...
    $this->CONF = array(
			...
			'partial_sum_label'=>$display_partial_sum_label,
  ...
  public function changeOrderStatusHook($status_id, $comment, $order_id) {
    ...
    $order_status_info = $this->model_invoicefox_order_status->getOrderStatus($status_id);
	...
	$invoicefox_id = $this->model_invoicefox_order->getInvoicefoxId($order_id); 
  ...
  private function makeInvoiceFromOrder($order_id) {
	...
	$products = $this->model_invoicefox_order->getOrderProducts($order_id);
	...
	$shipping = $this->findShipping($this->model_invoicefox_order->getOrderTotals($order_id));
	$coupon = $this->findCoupon($this->model_invoicefox_order->getOrderTotals($order_id));
	...
	if ($r2->isOk()) {    
	  $invA = $r2->getData();
	  $this->model_invoicefox_order->insertInvoicefoxId($order_id,$invA[0]['id']);
